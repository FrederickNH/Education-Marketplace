...
class CreateSeekingTutorsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('seeking_tutors', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('status', ['done', 'available','canceled'])->nullable();
            $table->timestamps();
        });
    }
    ...
    {
        Schema::table('seeking_tutors', function (Blueprint $table) {
            $table->dropForeign(['user_id']);
            $table->dropColumn('user_id');
            $table->dropForeign(['subject_id']);
            $table->dropColumn('subject_id');
        });
        Schema::dropIfExists('seeking_tutors');
    }
}
