...
class MessagesController extends Controller
{
    ...
    {
        $list = explode('-', $id, 2);
        $user = User::find($list[1]);
        $role = "";
        if($list[0] == 'school'){
            $school = School::where("user_id",$list[1])->first();
            $role = "school";
            $user->schoolName = $school->school_name;
            $user->picture = $school->picture;
        }elseif ($list[0] == 'shuttle') {
            $shuttle = SchoolShuttle::where("user_id",$list[1])->first();
            $role = "shuttle";
            $user->shuttleName = $shuttle->shuttle_name;
            $user->picture = $shuttle->picture;
        }elseif ($list[0] == 'organiser') {
            $organiser = CompetitionOrganiser::where("user_id",$list[1])->first();
            $role = "organiser";
            $user->orgName = $organiser->org_name;
            $user->picture = $organiser->picture;
        }elseif($list[0] == 'tutor'){
            $role = "tutor";
        }elseif ($list[0] == 'institution') {
            $institution = Tutor::where("user_id",$list[1])->first();
            $role = "institution";
            $user->institutionName = $institution->institution_name;
            $user->picture = $institution->institution_picture;
        }
