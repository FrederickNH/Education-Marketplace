        ...
        @foreach($awardList as $award)
            <div class="card card-profile shadow">
                <div class="card-body pt-0 pt-md-4">
                    <div class="row">     
                        <div class="col-md-4">
                            <img src="{{asset('assets')}}/img/dummy-certificate.png" class="card-img-top banner-img" alt="" />
                        </div>
                            ...
                            <div class="text-right">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        <a class="clear-btn dropdown-item text-warning" href="institutionAwardDelete/{{$award->id}}">Delete</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col">
                                <h2>{{__("Facility Name")}}: {{$award->title}}</h2>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="addForm" method="POST" action="{{ route('institutionAddAward') }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group{{ $errors->has('input-title') ? ' has-danger' : '' }}">
                                <label class="form-control-label text-white" for="input-title">{{ __('Award Title') }}</label>
                                <input type="text" name="input-title" id="input-title" class="form-control form-control-alternative{{ $errors->has('input-title') ? ' is-invalid' : '' }}"   placeholder="{{ __('Award Title') }}" required autofocus>
        
                                @if ($errors->has('input-title'))
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $errors->first('input-title') }}</strong>
                                    </span>
                                @endif
                            </div>
